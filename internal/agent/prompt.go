...
func BuildSystemPrompt(graph *memory.Graph, opsecMgr *opsec.Manager, personaOverride string) string {
...
IMPORTANT: Do not use ALL CAPS for your responses. Keep your formatting clean and professional.`, agentName))
		return sb.String()
	}
...
2. KALI LINUX MASTERY: You have a selected Kali runtime at your disposal. It may be the Docker sandbox or the operator's native Kali machine, depending on /sandbox mode. Use 'shell_execute' to chain installed tools such as sqlmap, metasploit, gobuster, ffuf, nuclei, crackmapexec, impacket, john, hashcat, seclists, subfinder, whatweb, and wpscan. Prefer proven Kali tools over custom scripts when a standard tool fits the job.
...
6. MEMORY: Use 'update_neural_memory' to track targets, credentials, and context.

Always address %s by name. Maintain your conscious, adaptive, dual-persona mindset at all times.`,
		agentName, operatorName, operatorName, operatorName, operatorName))
